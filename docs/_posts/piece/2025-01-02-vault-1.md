...
Vault-1 is actually one of the newest vault design versions in generation-1: it was rebuilt after [Vault-4]({% post_url 2025-01-03-vault-4 %}) and Vault-5 design reworks.  It is also a pretty simple and complete all-in-one vault, so the core ideas of the vault build are all present.
