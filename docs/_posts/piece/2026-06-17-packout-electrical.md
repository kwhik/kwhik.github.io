...
* Utilizing modern bluetooth control systems
* Using reasonably-sized wires (otherwise the wires themselves become a burden).  This was ultimately constrained to wires no bigger than 35mm² (2 AWG), allowing a bit over 100A comfortably.  [[1]](#1) 
* Having a clean connector system so the system can be assembled and disassembled as rapidly as possible [[2]](#2)


### Approach

The core of the approach described here is based on leveraging an unusually good match between the dimensions of the Packout™ Compact Tool Box and the size of the 206Ah LiFePO4 prismatic cell.  Four 206Ah cells easily fit in width, height, and depth within the compact toolbox.  This produces a 12V nominal battery with room for a BMS on top and connectors in the forward section.
